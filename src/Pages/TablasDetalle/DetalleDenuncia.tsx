...
// --- DetalleDenunciaPage.tsx estilizado correctamente ---
import { useEffect, useState } from "react"
import { useParams, useNavigate } from "react-router-dom"
import type { Denuncia } from "../../Types/Types"
import ApiService from "../../Components/ApiService"
...
export default function DetalleDenunciaPage() {
  ...
  const esImagen = (archivo: string): boolean => /\.(jpeg|jpg|png|gif)$/i.test(archivo)
  ...
  if (!denuncia) return (
    <div className="max-w-2xl mx-auto mt-8 bg-white shadow-lg rounded-lg p-6 text-center">
      <div className="py-8">
        ...
        <h2 className="text-2xl font-bold mb-2">Denuncia no encontrada</h2>
        <p className="text-gray-600 mb-6">No se pudo encontrar la información de esta denuncia.</p>
        <button onClick={() => navigate("/dashboard/denuncias")} className="bg-gray-600 text-white px-4 py-2 rounded-md hover:bg-gray-700 transition-colors flex items-center gap-2 mx-auto">
      ...
      <div className="p-6 space-y-6">
        <div className="flex flex-col lg:flex-row gap-6">
          <div className="flex-1 bg-white border border-gray-200 rounded-lg p-4">
            <h3 className="text-base font-semibold mb-4 flex items-center gap-2 text-gray-700">
              <User className="h-5 w-5 text-teal-600" /> Información del Denunciante
            </h3>
            <div className="grid gap-2">
              {[{ label: "Nombre", value: denuncia.nombreDenunciante || "Anónimo" },
                { label: "Identificación", value: denuncia.cedulaDenunciante || "Anónimo" },
                { label: "Notificación", value: denuncia.metodoNotificacion || "No especificado" },
                { label: "Medio", value: denuncia.medioNotificacion || "No especificado" }].map(({ label, value }) => (
                  <div key={label} className="grid grid-cols-[8rem_1fr] items-start gap-2">
                    <span className="text-gray-500">{label}:</span>
                    <span className="font-medium break-words whitespace-pre-wrap">{value}</span>
                  </div>
                ))}
            </div>
          </div>
          ...
          <div className="flex-1 bg-white border border-gray-200 rounded-lg p-4">
            <h3 className="text-base font-semibold mb-4 flex items-center gap-2 text-gray-700">
              <FileText className="h-5 w-5 text-teal-600" /> Detalles de la Denuncia
            </h3>
            <div className="grid gap-2">
              {[{ label: "Fecha", value: denuncia.Date ? formatearFecha(denuncia.Date) : "No disponible" },
                { label: "Tipo", value: denuncia.tipoDenuncia?.descripcion || "No especificado" },
                { label: "Descripción", value: denuncia.descripcion || "No especificado" },
                { label: "Lugar", value: denuncia.lugarDenuncia?.descripcion || "No especificado" },
                { label: "Ubicación exacta", value: denuncia.ubicacion || "No especificado" }].map(({ label, value }) => (
                  <div key={label} className="grid grid-cols-[8rem_1fr] items-start gap-2">
                    <span className="text-gray-500">{label}:</span>
                    <span className="font-medium break-words whitespace-pre-wrap">{value}</span>
                  </div>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          <h3 className="text-base font-semibold mb-3 flex items-center gap-2 text-gray-700">
            <File className="h-5 w-5 text-teal-600" /> Evidencias
          </h3>
        ...
        {isEditable && canEditDenuncia && (
          <div className="bg-white border border-gray-200 rounded-lg p-4">
            <h3 className="text-base font-semibold mb-4 flex items-center gap-2 text-gray-700">
              <UserCheck className="h-5 w-5 text-teal-600" /> Gestionar Estado de la Denuncia
            </h3>
            <p className="text-sm text-gray-600 mb-4">Puedes atender o denegar esta denuncia. No se enviará mensaje ya que puede ser anónima.</p>
            <div className="flex flex-col sm:flex-row gap-3">
              <button onClick={() => cambiarEstado("Aprobada")} className="flex-1 bg-teal-600 text-white px-4 py-2 rounded-md hover:bg-teal-700 flex items-center justify-center gap-2">
                <UserCheck className="h-5 w-5" /> Denuncia Atendida
              </button>
              <button onClick={() => cambiarEstado("Denegada")} className="flex-1 bg-red-600 text-white px-4 py-2 rounded-md hover:bg-red-700 flex items-center justify-center gap-2">
                <X className="h-5 w-5" /> Denuncia Denegada
              </button>
            </div>
          </div>
        )}
        ...
        <div className="text-right">
          <button onClick={() => navigate("/dashboard/denuncias")} className="bg-gray-600 text-white px-4 py-2 rounded-md hover:bg-gray-700 flex items-center gap-2 ml-auto">
            <ArrowLeft className="h-4 w-4" /> Volver a la lista
          </button>
        </div>
      </div>
