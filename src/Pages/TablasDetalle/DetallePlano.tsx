...
export default function DetallePlanoPage() {
        ...
        if (data.ArchivosAdjuntos) {
          try {
            const archivosParseados =
              typeof data.ArchivosAdjuntos === "string" ? JSON.parse(data.ArchivosAdjuntos) : data.ArchivosAdjuntos

            setArchivos(archivosParseados || [])
          } catch (error) {
            console.error("Error al procesar ArchivosAdjuntos:", error)
          }
        }
      } catch (error) {
        Swal.fire("Error", "Error al cargar la revisión del plano", "error")
  ...
  const manejarVerArchivo = (index: number) => {
    if (archivos[index]) {
      ...
      const fileUrl = `${ApiRoutes.urlBase}/${archivos[index].ruta.replace(/\\/g, "/")}`
      setArchivoVistaPrevia(fileUrl)
    }
  }
  ...
  if (!revisionPlano) {
    return (
      <div className="flex flex-col items-center justify-center h-screen">
        <p className="text-xl text-gray-700 mb-4">No se encontró la revisión de plano solicitada</p>
        <button
          onClick={() => navigate("/dashboard/planos")}
  ...
  const isEditable = revisionPlano.status === "Pendiente"
  const statusColor =
    revisionPlano.status === "Aprobada"
      ? "bg-green-100 text-green-800"
      : revisionPlano.status === "Denegada"
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-800">Detalles de la Revisión de Plano</h1>
        <button
          onClick={() => navigate("/dashboard/planos")}
          ...
          <div className="bg-white shadow-lg rounded-lg overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              <div className="flex justify-between">
                <span className="text-gray-600 font-medium">ID Revisión:</span>
                <span className="text-gray-800">{revisionPlano.id}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600 font-medium">Número de Expediente:</span>
                <span className="text-gray-800">{revisionPlano.NumeroExpediente || "No disponible"}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600 font-medium">Número de Plano:</span>
                <span className="text-gray-800">{revisionPlano.NumeroPlano || "No disponible"}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600 font-medium">Nombre:</span>
                <span className="text-gray-800">{revisionPlano.user?.nombre || "No disponible"}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600 font-medium">Apellidos:</span>
                <span className="text-gray-800">
                  {revisionPlano.user?.apellido1 || "No disponible"} {revisionPlano.user?.apellido2 || ""}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600 font-medium">Email:</span>
                <span className="text-gray-800">{revisionPlano.user?.email || "No disponible"}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-600 font-medium">Estado:</span>
                <span className={`px-3 py-1 rounded-full text-sm font-medium ${statusColor}`}>
                  {revisionPlano.status || "Pendiente"}
                </span>
              </div>
              <div className="mt-4">
                <span className="text-gray-600 font-medium block mb-2">Comentario:</span>
                <div className="bg-gray-50 p-4 rounded-lg">
                  <p className="text-gray-800 whitespace-pre-line">{revisionPlano.Comentario}</p>
                </div>
              </div>
            </div>
          </div>
                      ...
                      {archivos.map((archivo, index) => (
                        ...
                        >
                          <div className="flex items-center">
                            ...
                            <span className="text-gray-700">{archivo.nombre || `Documento ${index + 1}`}</span>
                          </div>
                  ...
                  <div className="flex items-center justify-between p-3 rounded-lg bg-blue-50 border border-blue-200 mb-4">
                    <div className="flex items-center">
                      ...
                      <span className="text-gray-700">
                        {archivos[archivoSeleccionado]?.nombre || `Documento ${archivoSeleccionado + 1}`} de{" "}
                        {archivos.length}
                      </span>
                    </div>
                    <div className="flex space-x-2">
